...
public final class X11NativeWindow: NativeWindow {
    internal unowned var displayServer: X11DisplayServer!
    ...
    func updateListeningEvents() {
        // smumriak: XInput2 stuff to remember: XI_MOTION event type IS ANY MOUSE MOVE. i.e. there's no way to filter mouse move when no mouse button is pressed

        // smumriak: what you see below is very unsafe code in swift, but it's more or less usual implicit cast for C code (without using unsafeBitCast). XInput2 is an old and poorly designed C API that aimed to be extensible (at the time), yet it came up very inconsistent. XISelectEvents function expects an input of event masks array which contains some number of descriptors represented by type XIEventMask. each of these descriptors contains identifier of the device and a *pointer* to CUnsignedChar containing the actual event mask to be used for filtering of the events from given device. XI_*MASK values are going up to 26 bits in length (despite the fact there are 32 events, i.e. mask needs at least 32 bits). we are facing the problem of "pointer to array containing structs, each containing pointer to some other memory". Volcano has DSL built specifically to solve this problem, but it's quite limited to Volcano itself. instead, code here does good old heap allocation of 32bit integers, casting pointer to be poining to 8bit integer, pass it to XISelectEvents function and deallocate previously allocated pointers. hope this does not break in future releases of swift

        var eventMasks: [XIEventMask] = displayServer.context.inputDevices
            .map { device in
                let mask: XInput2EventTypeMask

                if device.type == .pointer && ignoresMouseEvents {
                    mask = []
                } else {
                    mask = device.type.mask
                }

                let maskPointer = UnsafeMutablePointer<CUnsignedInt>.allocate(capacity: 1)
                maskPointer.initialize(to: UInt32(mask.rawValue))
        ...
        var xlibEventTypeMask = XlibEventTypeMask.geometry
        if acceptsMouseMovedEvents {
            xlibEventTypeMask.formUnion(.pointerMotion)
        }
        XSelectInput(display.pointer, windowIdentifier, xlibEventTypeMask.rawValue)
    }
